...
class XMLTagReplacer extends Replacer {
    constructor(tag, replace, {
        contentsOnly = true, // if should only replace the contents, leaving opener/closer intact
    } = {}) {
        const replacer = typeof replace === 'function' ? replace : () => replace;
        super(getXMLTagRegex(tag), async(matchedTag) => {
            const parsed = parseXMLTag(matchedTag);
            const { attributes, contents } = parsed;
            if (parsed.selfClosing && contentsOnly) {
                const newContents = await replacer(contents, { attributes });
            ...
            return contentsOnly
                ? parsed.opener + await replacer(contents, { attributes }) + parsed.closer
                : await replacer(matchedTag, { attributes });
        });
    }
}
